...
コード中に数値・文字列リテラルを直接埋め込まないこと。`src/lib/config.ts` の `APP_CONFIG` に定数として定義し、そこから参照する。
...
map.flyTo([lat, lng], APP_CONFIG.map.pickerFlyZoom)
```

変更が必要になったとき、`config.ts` を1箇所直すだけで全体に反映される。
