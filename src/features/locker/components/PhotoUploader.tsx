    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        try {
          ...
          if (!ctx) {
            logger.warn("[PhotoUploader] canvas context unavailable, skipping compression", {
              name: file.name,
            });
            resolve(file);
            return;
          ...
          canvas.toBlob(
            (blob) => {
              if (!blob) {
                logger.warn("[PhotoUploader] toBlob returned null, using original file", {
                  name: file.name,
                });
                resolve(file);
                return;
      ...
      img.onerror = (e) => {
        URL.revokeObjectURL(url);
        logger.warn("[PhotoUploader] failed to load image for compression", {
          name: file.name,
          type: file.type,
          error: e,
        });
    ...
    <div className="flex flex-col gap-3">
      ...
      {pending.length > 0 && (
        <div className="relative -mx-4 aspect-[4/3]">
          ...
          >
            {pending.map((p, i) => (
              <div key={p.id} className="bg-muted relative h-full w-full flex-shrink-0 snap-center">
            ...
            <>
              {currentIndex > 0 && (
                <button
                  type="button"
                  onClick={goPrev}
                  className="absolute top-1/2 left-2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-black/60 text-xl leading-none text-white"
                  aria-label="前へ"
                >
                  ‹
                </button>
              ...
              {currentIndex < pending.length - 1 && (
                <button
                  type="button"
                  onClick={goNext}
                  className="absolute top-1/2 right-2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-black/60 text-xl leading-none text-white"
                  aria-label="次へ"
                >
                  ›
                </button>
